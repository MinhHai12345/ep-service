package com.hai.minh.epservice.commons.constants;

public class URLConstants {

    public static final String SUBDIRECTORY_API = "/api";
    public static final String URL_CART = "/cart";

    public static final String URL_CUSTOMER = "/customer";

    // EP
    public static final String EP_CUSTOMER_URL = "/customers";

    public static final String OAUTH = "/oauth";

    public static final String ACCESS_TOKEN = "/access_token";

    public static final String EP_PRODUCT_URL = "/products";

    public static final String EP_PRODUCT_FILTER = "/products?filter=";

    public static final String EP_CART_URL = "/carts";
}